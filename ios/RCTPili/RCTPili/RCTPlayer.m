...
- (void) setOrientation:(NSString *) orientation
{
    if(orientation == _orientation){
        ...
        NSNumber* width =[NSNumber numberWithInteger:_plplayer.width];
        NSNumber* height =[NSNumber numberWithInteger:_plplayer.height];
        //        NSComparisonResult rs = [width compare:0];
        NSNumber* zero = [NSNumber numberWithInt:0];
        if([width isEqualToNumber:zero]) {
            
        }else{
            self.onResize(@{@"target":self.reactTag,@"width":width,@"height":height});
        }
    }
}
...
//    NSLog(state);
    switch (state) {
        ...
        case PLPlayerStatusPlaying:
            ...
            self.onResize(@{@"target":self.reactTag,@"width":[NSNumber numberWithInteger:player.width],@"height":[NSNumber numberWithInteger:player.height]});
            break;
